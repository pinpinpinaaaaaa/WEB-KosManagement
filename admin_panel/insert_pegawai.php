...
if (isset($_POST['submit'])) {
    ...
    $query_last_id = "SELECT id_pegawai FROM pegawai ORDER BY id_pegawai DESC LIMIT 1";
    $result_last_id = mysqli_query($con, $query_last_id);
    $row_last_id = mysqli_fetch_assoc($result_last_id);

    // Jika ada ID terakhir, buat ID baru, jika tidak mulai dari P001
    if ($row_last_id) {
        $last_id = $row_last_id['id_pegawai'];
        $number = (int) substr($last_id, 1); // Ambil angka dari ID terakhir
        $new_id = 'P' . str_pad($number + 1, 3, '0', STR_PAD_LEFT); // Format CXXX
    } else {
        $new_id = 'P001'; // ID pertama jika belum ada data
    }
    ...
    $insert_pegawai = "INSERT INTO PEGAWAI (id_pegawai, nama_pegawai, tanggal_mulai, gaji, username, password, role, alamat, no_telp, email) 
                        VALUES ('$new_id', '$nama_pegawai', '$tanggal_mulai', '$gaji', '$username', '$password', '$role', '$alamat', '$no_telp', '$email')";
